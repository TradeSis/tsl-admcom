...
do on error undo, return on endkey undo, retry:
    ...
    if vfuncod = 0 
    then do: 
        run get_passw.p (vsenha, output pok).
        if pok
        then next.
    end.
...
display "" with frame ff2
    ...
    if recatu1 = ?
    then
        find first APLICATIVO where aplicativo.ativo and
    ...
    repeat:
        find next APLICATIVO where aplicativo.ativo and
                   not can-find( aplifun where aplifun.funcod = func.funcod and
    ...
    repeat with frame frame-a:
        ...
        then do:
            find prev APLICATIVO where aplicativo.ativo and
                   not can-find( aplifun where aplifun.funcod = func.funcod and
                       aplifun.aplicod = aplicativo.aplicod) no-lock no-error.
            if not avail APLICATIVO
            then next.
