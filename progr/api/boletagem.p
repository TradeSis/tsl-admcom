...
def temp-table ttboleto no-undo serialize-name "titulo"
    ...
    field valor_nominal     as char 
    field data_emissao      as char 
    field id_titulo_empresa as char 
...
def temp-table ttmensagems no-undo serialize-name "mensagens"    
    field linha as int
    field texto as char.
...
def temp-table ttreturn no-undo serialize-name "boleto"
    ...
    field situacao_banrisul   as char
    field situacao_cip   as char
    field situacao_pagamento   as char
    field carteira   as char
    field cpf_cnpj_beneficiario   as char
    field cpf_cnpj_pagador   as char.
