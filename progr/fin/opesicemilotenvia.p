...
def var plotnum like sicred_contrato.lotnum.

def var poperacao   like sicred_contrato.operacao.
def var vtipocontratosicred as char.

update plotnum
    with frame fx row 5 width 80 side-labels.

find first sicred_contrato where sicred_contrato.lotnum = plotnum no-lock no-error.
if not avail sicred_contrato then do:
    message "Lote" plotnum "não encontrado".
    pause.
        next.
end.        

poperacao = sicred_contrato.operacao.
def var sx as log format "Sim/Nao".
message "Confirmar reenviar lote " poperacao plotnum update sx.
if not sx then return.
...
FUNCTION f-troca returns character
    ...
    assign vlote = plotnum.                         

    /* helio 24052023 */
    varqexp = "op" + string(today,"999999") + "f_" + string(vlote) + ".rem". /* normal op */
    
    find first sicred_contrato where sicred_contrato.lotnum   = plotnum no-lock no-error.
    if avail sicred_contrato 
    then do:
        find contrato where contrato.contnum = sicred_contr.contnum no-lock.
        if contrato.modcod begins "CP"
        then do:
            varqexp = "ep" + string(today,"999999") + "f_" + string(vlote) + ".rem". /* arquivo ep */
        end. 
    end.
    ...
    for each sicred_contrato where
        sicred_contrato.lotnum   = plotnum
        no-lock.
...
procedure p-registro-11.
  ...
  **/
  ...
  put unformat skip
    ...
    then  run p-grava-cobcod (input string(rowid(btitulo)), input vcobcoddes).
    */    
