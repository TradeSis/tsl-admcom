/* helio 14102022 - 3 tentativas caso esteja em loock o nosso numero */
...
def input parameter par-vlrservico    as dec.
...
def var vtenta as int . /* #14102022 */
...
function modulo11caixa returns integer
    (input par-numeron as char).
...
then do:
    find first banco where banco.numban = par-banco and
                    banco.situacao = yes no-lock no-error.
    if not avail banco
    then do:
...
then do on error undo:
    ...
    vagencia = trim(string(bancarteira.agencia)).
    if length(vagencia) = 5
    then vagencia = substring(vagencia,1,4).
    else vagencia = string(int(vagencia),"9999").
    vconta   = substring(string(bancarteira.contacor,"9999999"),1,6).
    vdvconta = substring(string(bancarteira.contacor,"9999999"),7,1).
    ...
    if banco.numban = 104 /* Caixa */
    then do:
        vDvNossoNumero = modulo11caixa("14" + 
                              string(vNossoNumero,"999999999999999")
                             ) .
    end.
    ...
    ASSIGN
        ...
        banBoleto.VlJuros        = 0
        banBoleto.TpBaixa        = ?.
        banboleto.vlservico      = par-vlrservico .
    ...
    if banco.numban = 104
    then do:
        banboleto.impnossonumero = "14" + string(banboleto.nossonumero,"999999999999999").
    end.
    ...
    then do:
        vcampo1       = string(banco.numban,"999")
                      + string(vnummoeda,"9")
                      + string(int(banboleto.bancart),"999")
                      +
                        string(
