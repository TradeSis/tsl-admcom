...
def temp-table ttmovimdoc no-undo
    ...
    index idx is unique primary 
        pagamento asc
        filtro asc ctmcod asc carteira asc
        modcod asc
        tpcontrato asc 
        etbcod asc
        cobcod asc.
...
disp
    ...
    repeat with frame frame-a:
        ...
        choose field vnome 
                ...
                help  "digite [C]ontratos/Parcelas"
        ...
        then leave bl-princ.
            ...
            then do:
                ...
                then run finct/telaanadocnov.p
                 (   vtitle + "/" + pfiltro,
                            poldfiltro, 
                            pfiltro,
                            ttmovimdoc.ctmcod,
                    ...
                    then do:
                        run finct/telaanaemictr.p (input ttmovimdoc.pagamento,
                                                                 input ttmovimdoc.cobcod,
        ...
        then do:
                ...
                if  pfiltro = "produto"
                then do:
                    run finct/telaanaliprod.p
                        (   ttmovimdoc.pagamento,
                            vtitle + "/" + pfiltro,
                            poldfiltro, 
                            pfiltro,
                            ttmovimdoc.ctmcod,
...
for each estab 
    ...
    no-lock.
        ...
        hide message no-pause.
        message color normal "fazendo calculos... aguarde..." string(time - vtime, "HH:MM:SS")
        ...
        for each pdvdoc use-index pagamentos where 
           ...
           vconta = vconta +  1. 
            ...
            find titulo where titulo.contnum = int64(pdvdoc.contnum) and
...
for each pdvtmov where pdvtmov.pagamento = no no-lock.
        ...
        message color normal "fazendo calculos... aguarde..." estab.etbcod
            pdvtmov.ctmnom string(time - vtime, "HH:MM:SS").
.
        ...
        for each pdvdoc use-index pagamentos where 
           ...
           vconta = vconta +  1. 
            ...
            hide message no-pause.
            message color normal "fazendo calculos... aguarde..." estab.etbcod
                pdvtmov.ctmnom vconta string(time - vtime, "HH:MM:SS").
            end.      
            vaux = int64(pdvdoc.contnum) no-error.
        ...
        if estab.etbcod = 200 /* helio 07122021 id 97063 */
        then do: /* pegar contratos CP1 do app, que grava direto */
            ...
            for each contrato where contrato.etbcod = estab.etbcod and
                                    ...
                                    no-lock.
                find first ttmovimdoc where
                    ttmovimdoc.pagamento = "Vendas APP" and
                    ttmovimdoc.filtro   = pfiltro and
                    ttmovimdoc.ctmcod   = "SITE" and
                    ttmovimdoc.carteira = pcarteira and
                    ttmovimdoc.modcod   = pmodcod  and
                ...
                then do:
                    create ttmovimdoc.
                    ttmovimdoc.pagamento = "Vendas APP".
                    ttmovimdoc.filtro = pfiltro.
                    ttmovimdoc.ctmcod = "SITE".
                    ttmovimdoc.carteira = pcarteira.
                    ttmovimdoc.modcod   = pmodcod.
